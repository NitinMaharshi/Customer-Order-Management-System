...
const ProductColor = () => {
    ...
    const fetchColorList = async () => {
        try {
            const response = await AxiosGet(`${API_URLS.ORDER.ORDER_LIST}`);
            if (response) {
        ...
        },
        {
            ...
            value: (cell) => (
                <div className='flex items-center justify-between gap-x-3'>
                    <FaEye size={18} className='cursor-pointer transition duration-300 hover:scale-125' onClick={() => navigate(`/orders/${cell?._id}`)} />
